...
def _ordered_absolute_messages(
    plan: tuple[MidiNotePlan, ...],
) -> tuple[_AbsoluteMessage, ...]:
    ...
    for item in _ordered_absolute_messages(plan):
        ...
        if delta_tick < 0:
            raise MidiEncodingError(
                "MIDI message ordering produced a negative delta time."
            )
        track.append(
            mido.Message(
                item.message_type,
...
def _parsed_note_messages(track: Any) -> tuple[tuple[int, str, int, int, int], ...]:
    ...
    for message in track:
        if (
            isinstance(message.time, bool)
            or not isinstance(message.time, int)
            or message.time < 0
        ):
            raise InvalidMidiArtifactError(
                "Parsed MIDI delta times must be non-negative integers."
            )
        absolute_tick += message.time
        if message.type in {"note_on", "note_off"}:
            if not 0 <= message.note <= 127 or not 0 <= message.velocity <= 127:
                raise InvalidMidiArtifactError(
                    "Parsed MIDI note data is outside the valid range."
                )
            messages.append(
                (
                    absolute_tick,
...
def _validate_encoded_content(
    ...
    *,
    ...
    try:
        ...
        raise InvalidMidiArtifactError(
            "encoded content could not be parsed as MIDI"
        ) from error
    ...
    if metadata_types != ("track_name", "set_tempo", "end_of_track"):
        raise InvalidMidiArtifactError(
            "Encoded MIDI metadata track is incomplete or out of order."
        )
    ...
    if parsed.tracks[0][1].tempo != settings.tempo_microseconds_per_beat:
        raise InvalidMidiArtifactError(
            "Encoded MIDI tempo does not match export settings."
        )
    ...
    if (
        note_track[0].name != _CONCERT_TRACK_NAME
        or note_track[-1].type != "end_of_track"
    ):
    ...
    if sum(message[1] == "note_on" for message in parsed_messages) != len(plan):
        raise InvalidMidiArtifactError(
            "Encoded MIDI note_on count does not match the plan."
        )
    if sum(message[1] == "note_off" for message in parsed_messages) != len(plan):
        raise InvalidMidiArtifactError(
            "Encoded MIDI note_off count does not match the plan."
        )
        ...
        *,
        ...
        try:
            midi_file = mido.MidiFile(
                type=MIDI_FILE_TYPE, ticks_per_beat=MIDI_TICKS_PER_BEAT
            )
            ...
            note_track.append(
                mido.MetaMessage("track_name", name=_CONCERT_TRACK_NAME, time=0)
            )
